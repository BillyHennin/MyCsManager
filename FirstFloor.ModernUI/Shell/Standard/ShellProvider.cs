...
namespace FirstFloor.ModernUI.Shell.Standard
{
    ...
    internal interface IApplicationAssociationRegistration
    {
        ...
        bool QueryAppIsDefault([MarshalAs(UnmanagedType.LPWStr)] string pszQuery, AT atQueryType, AL alQueryLevel,
    ...
    {
        void ParseDisplayName([In] IntPtr hwnd, [In] IBindCtx pbc, [In] [MarshalAs(UnmanagedType.LPWStr)] string pszDisplayName, [In] [Out] ref int pchEaten,
        ...
        object GetUIObjectOf([In] IntPtr hwndOwner, [In] uint cidl,
            [In] [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.SysInt, SizeParamIndex = 2)] IntPtr apidl, [In] ref Guid riid,
            [In] [Out] ref uint rgfReserved);

        void GetDisplayNameOf([In] IntPtr pidl, [In] SHGDN uFlags, [Out] out IntPtr pName);

        void SetNameOf([In] IntPtr hwnd, [In] IntPtr pidl, [In] [MarshalAs(UnmanagedType.LPWStr)] string pszName, [In] SHGDN uFlags, [Out] out IntPtr ppidlOut);
    }
